<template lang="pug">
div
  div.row
    div.col-md-12
      p#error(v-if="!connected && hasConnected") Socket server is disconnected
  div.row
    div.col-md-9
      div.panel.panel-default
        div.panel-body
          div#control.float-middle
            div.row
              div.col-sm-2.col-sm-offset-2
                input.form-control(v-model="ability" placeholder="Ability dice")
              div.col-sm-2">
                input.form-control(v-model="bonus" placeholder="Bonus dice")
              div.col-sm-2">
                input.form-control(v-model="static" placeholder="Static +X")
              div.col-sm-2">
                button.btn.btn-primary(v-on:click="roll") Roll
          hr
          div#history
            table.table.table-hover
              thead
                tr
                  th When
                  th Who
                  th Dice
                  th Result
              tbody
                tr(is="entry" :entry="entry" v-for="entry in history")
    div.col-md-3
      div.panel.panel-default
        div.panel-heading
          div.clearfix
            h3.pull-left.panel-title People
            a#btn_leave(v-on:click="leave")
              span.pull-right.label.label-danger Leave
        div.panel-body
          div
            p#users
              span(v-for="name in activeUsers") {{ name }}
              br
      div.panel.panel-default
        div.panel-heading
          h3.panel-title How to use this app
        div.panel-body
          p.
            In the three textboxes at the top of the screen in the middle, input the number of dice
            you have for the test's relevant <strong>ability</strong> in the first box, the number
            of any <strong>bonus</strong> dice you get to add to the test in the second, and any
            static modifier in the third.
            Then, click <strong>Roll</strong>.
            <br><br>
            Your rolls and everyone else's rolls will appear in the middle of the screen in reverse
            chronological order (so the most recent rolls are at the top). The "Result" column shows your 
            results in the order <code>raw rolls -> top dice you keep -> final value</code>.
            <br><br>
            If you need to change your name, use the <strong>Leave</strong> link in the upper-right of the
            screen to go back to the sign-in page.
</template>

<script>
import Vue from 'vue'
import Entry from './Entry.vue'
import config from './config.js'

export default {
  data () {
    return {
      history: [],
      activeUsers: [],
      ability : 2,
      bonus : 0,
      static : 0,
      connected: false,
      hasConnected: false,
      name: ''
    }
  },
  components: {
    'entry': Entry
  },
  created() {
    this.name = sessionStorage.getItem('name')
  },
  ready() {
    this.getHistory()
    this.$socket.emit('setname', {name: this.name}, function() {
    })
  },
  sockets: {
    connect: function() {
      this.connected = true
      this.hasConnected = true
    },
    disconnect: function() {
      this.connected = false
    },
    roll_event: function() {
      this.getHistory()
    },
    users: function(data) {
      this.activeUsers = data.users
    }
  },
  methods: {
    getHistory: function() {
      this.$http.get(config['api_url'] + 'history').then((response) => {
        this.$set('history', response.data.history)
      })
    },
    roll: function() {
      this.$socket.emit('roll_request', {
        ability: this.ability,
        bonus: this.bonus,
        static: this.static
      })
    },
    leave: function() {
      let router = this.$router
      this.$socket.emit('leave', function() {
        router.go('/')
      })
    }
  }
}
</script>

<style lang="stylus">
body
  padding-top 1em
  background-color rgba(0, 0, 0, 0.02)
  font-family 'Roboto', sans-serif
  font-size 1.5em

div.container-large
  width 90%
  margin-left auto
  margin-right auto

span.loading
  color gray

h3.nomartop
  margin-top 0

a#btn_leave
  cursor pointer

p#error
  background-color rgba(255, 0, 0, 0.5)
  padding 5px
  color black
  font-size 1.5em

.fade-transition
  transition opacity .5s ease

.fade-enter, .fade-leave
  opacity 0

div.container > h2
  margin-top 0
</style>
